...
class Qwen2_5OmniGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        # 修改 Qwen2_5OmniGRPOTrainer.__init__ 方法中的 processing_class 初始化部分
        if processing_class is None:
            if "Qwen2.5-Omni" in model_id:
                # 直接使用 Qwen2_5OmniProcessor 而非 AutoProcessor
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # 然后将文本字符串传递给处理器的text参数，并指定dtype
        prompt_inputs = self.processing_class(
            text=text,  # 传递处理过的文本字符串
            audio=audios,
            images=images, 
            videos=videos,
            return_tensors="pt",
        ...
        if inputs[0]['data_type'] == 'video':
            # 确定复制倍数
            repeat_factor = len(prompt_completion_ids)  # 这是生成的回答数量

            # 处理视频相关张量
            if "pixel_values_videos" in prompt_inputs:
                prompt_inputs["pixel_values_videos"] = prompt_inputs["pixel_values_videos"].repeat_interleave(repeat_factor, dim=0)
                if "video_grid_thw" in prompt_inputs:
                    prompt_inputs["video_grid_thw"] = prompt_inputs["video_grid_thw"].repeat_interleave(repeat_factor, dim=0)
                if "video_second_per_grid" in prompt_inputs:
                    prompt_inputs["video_second_per_grid"] = prompt_inputs["video_second_per_grid"].repeat_interleave(repeat_factor)
